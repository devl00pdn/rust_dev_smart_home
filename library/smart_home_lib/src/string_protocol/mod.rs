...
const HANDSHAKE_REQUEST: &str = "hi_server";
const HANDSHAKE_RESPOND: &str = "hi_client";
const WRAPPING_SYMBOLS: &str = "@@";

pub struct Protocol {}

impl Protocol {
    pub fn handshake_request_msg() -> String {
        Self::wrap_message(HANDSHAKE_REQUEST)
    }

    pub fn handshake_respond_msg() -> String {
        Self::wrap_message(HANDSHAKE_RESPOND)
    }

    pub fn unwrap_message(raw_msg: &str) -> Result<Vec<String>, String> {
    ...
    fn wrap_message(msg: &str) -> String {
        let mut wrapped = WRAPPING_SYMBOLS.to_string();
        wrapped.insert_str(0, msg);
...
mod tests {
    ...
    #[test]
    fn check_wrapping() {
        assert_eq!("@@abc@@", Protocol::wrap_message("abc"));
    }

    #[test]
    fn check_handshake_request_msg() {
        assert_eq!("@@hi_server@@", Protocol::handshake_request_msg());
    }

    #[test]
    fn check_handshake_resp_msg() {
        assert_eq!("@@hi_client@@", Protocol::handshake_respond_msg());
    }


    #[test]
    fn check_unwrap_once() {
        if let Ok(msg) = Protocol::unwrap_message("@@some_text@@") {
            assert_eq!(msg.len(), 1);
    ...
    fn check_unwrap_multiple() {
        if let Ok(msg) = Protocol::unwrap_message("@@some_text1@@@@some_text2@@@@some_text3@@") {
            assert_eq!(msg.len(), 3);
    ...
    fn check_unwrap_parsing_err() {
        if let Err(msg) = Protocol::unwrap_message("@some_text1@@") {
